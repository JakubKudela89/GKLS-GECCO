...
if ((error_code=GKLS_set_default()) != GKLS_OK) {
  ...
  GKLS_dim = dim;                                             
  GKLS_num_minima = num_min;                                     
  if ((error_code = GKLS_domain_alloc()) != GKLS_OK){        
    return error_code;
  }

  GKLS_global_dist = global_dist;
  GKLS_global_radius = global_radius*GKLS_global_dist;
...
for (func_num=1; func_num <= num_problems; func_num++)
{
  ...
  for (i=0; i<GKLS_num_minima; i++) {
    ...
    for (j=0; j<GKLS_dim; j++)
	 fprintf(GKLS_info,"%12.10f ",GKLS_minima.local_min[i][j]);
    fprintf(GKLS_info,") = %12.10f;  ",GKLS_minima.f[i]);
	fprintf(GKLS_info,"rho[%u] = %12.10f.\n", i+1, GKLS_minima.rho[i]);
  }
  ...
  //if (GKLS_dim == 2) {
    ...
    for (i=0; i<GKLS_num_minima; i++) {
      for (j=0; j<GKLS_dim; j++)
      fprintf(min_info,"%12.10f ",GKLS_minima.local_min[i][j]);
	    fprintf(min_info,"%12.10f\n",GKLS_minima.f[i]);
	  }
...
if ( (xx=(double *)malloc((size_t)GKLS_dim*sizeof(double))) == NULL)
 ...
 sprintf(filename,"data/grad_hes/deriv%03d.txt",func_num);
